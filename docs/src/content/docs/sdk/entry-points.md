...
| V-013 | [§5.7](/sdk/execution-primitives/#57-select_response) | All regular expressions are syntactically valid RE2. |
| V-014 | [§5.7](/sdk/execution-primitives/#57-select_response) | All CEL expressions are syntactically valid (parse without error). |
| V-015 | [§5.7](/sdk/execution-primitives/#57-select_response) | All JSONPath expressions are syntactically valid. |
| V-016 | [§5.7](/sdk/execution-primitives/#57-select_response) | All template references use valid syntax (no unclosed `{{`). Escaped sequences (`\{{`) are not template references and MUST NOT be flagged. |
| V-017 | [§4.3](/sdk/evaluation/#43-evaluate_expression) | `severity.confidence` is in range 0–100 when present. |
| V-018 | [§7](/sdk/diagnostics/) | Indicator `surface` is valid for the indicator's resolved protocol. |
| V-019 | [§5.3](/sdk/execution-primitives/#53-evaluate_condition) | Trigger `count` and `match` are only present when `event` is also present. |
...
| V-023 | [§4.2](/sdk/evaluation/#42-evaluate_pattern) | `attack.id`, when present, matches the pattern `^[A-Z][A-Z0-9-]*-[0-9]{3,}$`. |
| V-024 | [§6.1](/sdk/extension-points/#61-celevaluator) | Each explicitly specified `indicator.id`, when `attack.id` is present, matches the pattern `^[A-Z][A-Z0-9-]*-[0-9]{3,}-[0-9]{2,}$` AND its prefix (the portion before the final `-NN` segment) equals `attack.id`. For example, indicator `ACME-003-02` is valid in attack `ACME-003` but invalid in attack `ACME-007`. When `attack.id` is absent, explicitly specified indicator IDs are accepted without pattern constraints but MUST still be unique (V-010). |
| V-025 | [§6.1](/sdk/extension-points/#61-celevaluator) | `indicator.confidence`, when explicitly present, is in range 0–100 inclusive. |
| V-026 | [§6.3](/sdk/extension-points/#63-generationprovider) | All `expression.variables` values are syntactically valid simple dot-path expressions per the grammar in [§5.1.1](/sdk/execution-primitives/#511-simple-dot-path). No wildcards or indices. These values are resolved via `resolve_simple_path` at evaluation time ([§4.3](/sdk/evaluation/#43-evaluate_expression)) and malformed paths should be caught early. |
...
| V-042 | [§5.2](/sdk/execution-primitives/#52-parse_duration), [§5.3](/sdk/execution-primitives/#53-evaluate_condition) | Trigger MUST specify at least one of `event` or `after`. An empty trigger object is invalid. |
| V-043 | [§5.2](/sdk/execution-primitives/#52-parse_duration) | Binding-specific action objects (those containing no known action key) MUST contain exactly one non-`x-` key. |
