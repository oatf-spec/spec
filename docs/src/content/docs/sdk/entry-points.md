...
| V-029 | [§7](/specification/protocol-bindings/) | For recognized modes (v0.1: `mcp_server`, `mcp_client`, `a2a_server`, `a2a_client`, `ag_ui_client`), trigger event types listed in the Event-Mode Validity Registry ([§2.22](/sdk/core-types/#222-event-mode-validity-registry)) MUST be valid for the actor's mode. Event types not listed in the registry on a recognized mode SHOULD produce a warning but MUST NOT be rejected. For unrecognized modes, skip event validation. |
